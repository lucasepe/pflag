...
func TestEverything(t *testing.T) {
	...
	Int("test_optional_int", 0, "optional int value")
	...
	VisitAll(visitor)
	if l := len(m); l != 9 {
		t.Errorf("VisitAll misses some flags (l=%d)", l)
		for k, v := range m {
			t.Log(k, *v)
		}
	}
...
func TestShorthandLookup(t *testing.T) {
	...
	if flag == nil {
		t.Fatalf("f.ShorthandLookup(\"a\") returned nil")
	}
	if flag.Name != "boola" {
...
func TestSetOutput(t *testing.T) {
	...
	if out := buf.String(); !strings.Contains(out, "Usage of test") {
